            ...
            ZStack {
                ...
                if showBSOD {
                    ZStack {
                        ...
                        // Add X button that appears after 1 second
                        if showXButton {
                            Button(action: {
                                withAnimation {
                                    showMessage = true
                    ...
                    .onAppear {
                        ...
                        // Transition to SecretLevel after BSOD fades
                        DispatchQueue.main.asyncAfter(deadline: .now() + 4.0) {
                            withAnimation(.easeInOut(duration: 0.5)) {
                                showBSOD = false
                                // Transition to SecretLevel instead of progress map
                                levelManager.currentState = .secretLevel
                            }
                        }
                    }
                    .onDisappear {
                        // Reset states when BSOD disappears
                        showXButton = false
                        showMessage = false
                    }
                }
